...
$currentPage=basename($path,'.php');
			...
			<a href="friends.php">Friend Request</a> ||
			<a href="my_friends.php">Friend List</a> ||
			<a <?php
						if($currentPage=='send_request') {
							echo 'id="active"';
						}?>href="send_request.php">Send Friend Request</a> ||
		...
		<table width="550" align="center">
			<tr align="center">
				<td colspan="4"><h2>Send Friend Request:</h2></td>
			</tr>
			<tr>
				<th>SN.:</th>
				<th>Send To:</th>
		...
		global $con;
			...
			$user_id = $row['user_id'];
			$req = mysqli_query($con,"SELECT to_id FROM frnd_req WHERE from_id='$user_id'");
			$sn =0;
			while($run_req= mysqli_fetch_array($req)){
				$to_id = $run_req['to_id'];
				

			$get_sender =" select * from users where user_id='$to_id' ";
			$run_sender = mysqli_query($con,$get_sender);
			$row = mysqli_fetch_array($run_sender);
		...
		?>
			<tr align="center">
				<td><?php echo $sn;  ?></td>
				<td>
				<a target="blank" href="user_profile.php?user_id=<?php echo $to_id; ?>" >
...
if($to_id){
		...
		$to= mysqli_query($con,"SELECT id FROM frnd_req WHERE from_id='$user_id'" );


	if(mysqli_num_rows($to)==1){
			echo "<a style='color:red;font-weight:bold' href='cancel_send_request.php?cancel=cancel&user_id=$to_id'>X</a>";
