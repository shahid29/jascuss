...
$currentPage=basename($path,'.php');
		...
		<p align="center">
			...
			<a <?php
						if($currentPage=='friends') {
							echo 'id="active"';
						}?>href="friends.php">Friend Request</a> ||
			<a href="my_friends.php">Friend List</a> ||
			<a href="send_request.php">Send Friend Request</a> ||
			<a href="online_frnd.php">Online Friends</a>
		...
		<table width="550" align="center">
			<tr align="center">
				<td colspan="4"><h2>Friend Request List:</h2></td>
			</tr>
			<tr>
				<th>SN.:</th>
				<th>Sender:</th>
		...
		global $con;
			...
			$user_id = $row['user_id'];
			$req = mysqli_query($con,"SELECT from_id FROM frnd_req WHERE to_id='$user_id'");
			$sn =0;
			while($run_req= mysqli_fetch_array($req)){
				$from_id = $run_req['from_id'];
				

			$get_sender =" select * from users where user_id='$from_id' ";
			$run_sender = mysqli_query($con,$get_sender);
			$row = mysqli_fetch_array($run_sender);
		...
		?>
			<tr align="center">
				<td><?php echo $sn;  ?></td>
				<td>
				<a target="blank" href="user_profile.php?user_id=<?php echo $from_id; ?>" >
...
if($from_id){
		...
		$from= mysqli_query($con,"SELECT id FROM frnd_req WHERE from_id='$from_id'" );
		
		if(mysqli_num_rows($from)==1){
			echo "<a target='blank' style='color:green;font-weight:bold;' href='accept_friend.php?add=accept&user_id=$from_id'>Accetp</a>";
...
if($from_id){
	...
	if(mysqli_num_rows($from)==1){
			echo "<a style='color:red;font-weight:bold' href='accept_friend.php?add=ignore&user_id=$from_id'>X</a>";
