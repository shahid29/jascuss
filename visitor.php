...
$currentPage=basename($path,'.php');
			...
			<a <?php
						if($currentPage=='my_friends') {
							echo 'id="active"';
						}?>href="my_friends.php">Friend List</a> ||
		...
		<table width="550" align="center">
			<tr align="center">
				<td colspan="4"><h2>People Are View Your Profile:</h2></td>
			</tr>
			<tr>
				<th>SN.:</th>
				<th>Name:</th>
...
<?php
		...
		global $con;
			...
			$whoVisit = mysqli_query($con,"SELECT *  FROM visitor  WHERE visited_id='$user_id1'");
			$sn =0;

			while($run_visit= mysqli_fetch_array($whoVisit)){
				$user_id = $run_visit['user_id'];
				$name    = $run_visit['user_id_name'];
		...
		<?php
			...
			if (isset($_GET['delVisit'])) {
				$user_id=$_GET['delVisit'];
				$delVisit = mysqli_query($con,"DELETE FROM visitor WHERE user_id='$user_id'");
...
?>	
			 ...
			 <a onclick="return confirm('Are Your Sure!')" style="color:red" href="?delVisit=<?php echo $user_id; ?>" >X</a>
